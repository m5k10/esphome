...
void ESP32BLETracker::setup() {
  ...
  ExternalRAMAllocator<esp_ble_gap_cb_param_t::ble_ext_adv_report_param> allocator_ext(
      ExternalRAMAllocator<esp_ble_gap_cb_param_t::ble_ext_adv_report_param>::ALLOW_FAILURE);
  this->scan_result_buffer_ext_ = allocator_ext.allocate(ESP32BLETracker::SCAN_RESULT_BUFFER_SIZE);
  if (this->scan_result_buffer_ext_ == nullptr) {
    ESP_LOGE(TAG, "Could not allocate buffer for BLE ext Tracker!");
    this->mark_failed();
  }
...
void ESP32BLETracker::loop() {
  ...
  if (this->scanner_state_ == ScannerState::RUNNING &&
      this->scan_result_ext_index_ &&  // if it looks like we have a scan result we will take the lock
      xSemaphoreTake(this->scan_result_lock_, 5L / portTICK_PERIOD_MS)) {
    uint32_t index = this->scan_result_ext_index_;
    if (index >= ESP32BLETracker::SCAN_RESULT_BUFFER_SIZE) {
      ESP_LOGW(TAG, "Too many BLE events to process. Some devices may not show up.");
    }
    ...
    if (this->raw_advertisements_) {
      for (auto *listener : this->listeners_) {
        listener->parse_devices(this->scan_result_buffer_ext_, this->scan_result_ext_index_);
      }
      for (auto *client : this->clients_) {
        client->parse_devices(this->scan_result_buffer_ext_, this->scan_result_ext_index_);
      }
    }
    ...
    if (this->parse_advertisements_) {
      for (size_t i = 0; i < index; i++) {
        ESPBTDevice device;
        device.parse_scan_rst(this->scan_result_buffer_ext_[i]);
...
void ESP32BLETracker::start_scan_(bool first) {
  ...
  if (scan_ret) {
    ESP_LOGE(TAG, "esp_ble_gap_set_ext_scan_params failed: %d", scan_ret);
    return;
  }
}
...
void ESP32BLETracker::gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
  switch (event) {
    ...
    case ESP_GAP_BLE_EXT_ADV_REPORT_EVT:
      this->gap_scan_result_(param->ext_adv_report);
      break;
...
void ESPBTDevice::parse_scan_rst(const esp_ble_gap_cb_param_t::ble_ext_adv_report_param &param) {
  this->scan_result_ext_ = param;
  for (uint8_t i = 0; i < ESP_BD_ADDR_LEN; i++)
    this->address_[i] = param.params.addr[i];
  this->address_type_ = (esp_ble_addr_type_t) param.params.addr_type;
  this->rssi_ = param.params.rssi;
  ...
  ESP_LOGVV(TAG, "  Adv data: %s", format_hex_pretty(param.params.adv_data, param.params.adv_data_len).c_str());
#endif
}
...
void ESPBTDevice::parse_adv_(const esp_ble_gap_cb_param_t::ble_ext_adv_report_param &param) {
  size_t offset = 0;
  const uint8_t *payload = param.params.adv_data;
  uint8_t len = param.params.adv_data_len /**+ param.scan_rsp_len**/;
